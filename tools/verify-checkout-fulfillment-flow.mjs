...
for (const method of METHODS) {
  ...
  if (method === "store_pickup_self") {
    assert("store pickup hides vehicle", !panels.vehicle && panels.storePickup);
    assert("store pickup vehicle not required", !checkoutVehicleInfoRequired(method));
  }
}
...
assert("store pickup valid without vehicle/address", storePickupValid);
