...
export async function loadAdminNavBadges(): Promise<AdminNavBadges> {
  ...
  const { pendingInquiries, pendingBatteryTalk } = snapshot.consultationSummary;

  return {
    [ADMIN_ROUTES.orders]: snapshot.ordersNavBadge,
    [`${ADMIN_ROUTES.inquiries}?type=consultation`]: pendingBatteryTalk || undefined,
    [ADMIN_ROUTES.inquiries]: pendingInquiries || undefined,
  };
}
