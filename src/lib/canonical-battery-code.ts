...
export function canonicalBatteryCode(raw: string | null | undefined): string {
  ...
  const fromAlias = getCanonicalBatteryCode(trimmed);
  const candidate = fromAlias || trimmed;
  return (
    productBatteryCode(candidate) ||
    productBatteryCode(trimmed) ||
    normalizeBatteryCode(candidate) ||
