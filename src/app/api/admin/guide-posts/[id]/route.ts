...
export async function PATCH(
  ...
  let body: Partial<GuidePostInput>;
  try {
    body = (await request.json()) as Partial<GuidePostInput>;
  } catch {
  ...
  try {
    const item = await updateGuidePost(id, body);
    if (!item) {
      return NextResponse.json({ ok: false, message: "가이드를 찾을 수 없습니다." }, { status: 404 });
    }
    ...
    return NextResponse.json({ ok: false, message: "가이드를 수정하지 못했습니다." }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const ok = await softDeleteGuidePost(id);
    if (!ok) {
      return NextResponse.json({ ok: false, message: "가이드를 찾을 수 없습니다." }, { status: 404 });
    }
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ ok: false, message: "가이드를 삭제하지 못했습니다." }, { status: 500 });
  }
}
