...
export function VehicleDetailSidebar({
  ...
  return (
    <aside className="space-y-3 lg:sticky lg:top-[72px] lg:self-start">
      {!hideBatteryLinks ? (
      <SidebarPanel title={multiFuel ? "규격 바로가기" : "이 차량 추천 배터리"}>
      ...
      <SidebarPanel title="관련 가이드">
        {guides.length === 0 ? (
          <Link className="text-xs font-black text-[var(--bm-primary)] hover:underline" href="/guides">
            전체 가이드 보기 →
          </Link>
        ) : (
          ...
          <div className="space-y-2">
            {relatedVehicles.map((v) => (
              ...
              >
                ...
                <p className="mt-0.5 text-[10px] font-bold text-[var(--bm-primary)]">{v.battery}</p>
              </Link>
            ))}
          </div>
        )}
