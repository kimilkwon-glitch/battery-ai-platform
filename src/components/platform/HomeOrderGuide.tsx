...
export function HomeOrderGuide() {
  return (
    <HomeSectionShell rhythm="order" data-section="order-guide">
      <p className={`${bm.label} !text-emerald-800`}>오주문 방지</p>
      ...
      <ul className="mt-3 list-none space-y-2 p-0">
        ...
        <li className={bm.stepItem}>
          ...
          <p className="text-sm font-medium leading-relaxed text-slate-700">
            확실하지 않으면 체크리스트와 사진으로 한 번 더 확인하는 것이 좋습니다.
          </p>
        </li>
      </ul>
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        ...
        <Link className={`${bm.btnSecondary} justify-center text-sm`} href={HUB_PHOTO_CHECK}>
          사진 확인 안내
        </Link>
        <Link className={`${bm.btnGhost} text-sm`} href={HUB_PHOTO}>
          사진으로 확인
        </Link>
      </div>
    </HomeSectionShell>
