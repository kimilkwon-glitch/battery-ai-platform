        ...
        <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-1 flex flex-wrap gap-1.5">
            {SPEC_SEARCH_CHIPS.map((code) => (
              <Link
                key={code}
                className="rounded-full bg-[var(--bm-primary)] px-2.5 py-1 text-[10px] font-black text-white shadow-sm transition hover:opacity-90"
            ...
            {HERO_BATTERY_CODES.filter((c) => !SPEC_SEARCH_CHIPS.includes(c as (typeof SPEC_SEARCH_CHIPS)[number])).map(
              (code) => (
                <Link
                  key={code}
                  className="rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-800"
