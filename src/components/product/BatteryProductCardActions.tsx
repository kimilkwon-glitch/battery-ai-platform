...
const ACTION_ORDER_COMPACT =
  "home-spec-card-action home-spec-card-action--order inline-flex w-full min-h-[2.5rem] cursor-pointer items-center justify-center rounded-lg px-3 py-2 text-[0.8125rem] font-black transition sm:min-h-[2.625rem] sm:text-sm";
...
export function BatteryProductCardActions({
  ...
  const reviewBtn = `${ACTION_BASE} bg-emerald-100 text-emerald-900 ring-1 ring-emerald-200/90 hover:bg-emerald-200`;
  const specBtn = clsx(
    ACTION_BASE,
    tone === "catalog" ? "home-spec-card-action--spec" : "bg-blue-50 text-blue-950 ring-1 ring-blue-200/90 hover:bg-blue-100",
  );
  ...
  if (compact) {
    return (
      <div
        className="home-spec-card-actions home-spec-card-actions--compact flex flex-col gap-2"
        data-product-card-actions={links.batteryCode}
      >
        {orderControl}
        <div className="home-spec-card-actions__text-links">
          <Link href={links.batterySpecGuideHref} className="home-spec-card-text-link">
            규격 보기
          </Link>
          <span className="home-spec-card-text-link__sep" aria-hidden>
            ·
          </span>
          <Link href={links.reviewHref} className="home-spec-card-text-link">
            리뷰 보기
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="home-spec-card-actions flex flex-col gap-2" data-product-card-actions={links.batteryCode}>
